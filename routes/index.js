...
const filePaths = [
    path.join(__dirname, "../data/bedrooms.json"),
    path.join(__dirname, "../data/lodging.json"),
    path.join(__dirname, "../data/guests.json"),
    
];

let bedrooms = new Map();
let lodging = new Map();
let guests = new Map();


const fileNames = [bedrooms, lodging, guests];
...
const getbedroomById = (productId) => {
...
router.get('/products', (req, res) => {
    ...
    res.render('products', { title: 'Habitaciones', product: fileNames[0], totalStock });
});
...
router.get('/sales', (req, res) => { res.render('sales', { title: 'Alquilar', sale: fileNames[1], product: fileNames[0], supplier: fileNames[2], getProductById: getbedroomById, getSupplierById, getTypeofSale }) })
router.get('/suppliers', (req, res) => res.render('suppliers', { title: 'Huespedes', supplier: fileNames[2] }))
...
router.get('/descargar-excel', (req, res) => {
    try {
        ...
        const transacciones = Array.from(fileNames[1].values()).map(transaction => {
            ...
            const producto = getbedroomById(transaction.productId).name;
